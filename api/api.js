...
// Get all tasks for a user (no date filtering)
app.get('/user/:user_id/all-tasks', async (req, res) => {
  try {
    const { user_id } = req.params;
    ...
    // Then get all tasks for those plants
...
// ========================
// TASK ROUTES WITH DATE FILTERING FOR TASK TAB
// ========================
...
// To add a plant for a user and create propagation tasks
...
async function getPlantWateringInterval(userPlantId) {
  try {
    ...
    const wateringString = userPlant.plant_id.watering; 
