...
class _CareGuidePageState extends State<CareGuidePage> with TickerProviderStateMixin {
  ...
  final Map<String, bool> _sectionStates = {
    ...
    'water': false,
    'pruning': false,
    'harvest': false, // Added harvest section
  };

  // Harvest data from Supabase task_template
  List<Map<String, dynamic>> harvestTemplates = [];
  bool _isLoadingHarvest = true;
  ...
  Future<void> _loadHarvestData() async {
    try {
      // Load task templates from Supabase and filter for harvest-related categories
      final response = await Supabase.instance.client
          .from('task_template')
          .select('*')
          .inFilter('category', ['Harvesting', 'Harvest']);
      
      if (response != null) {
        setState(() {
          harvestTemplates = List<Map<String, dynamic>>.from(response);
  ...
  Widget _buildHarvestContent() {
    ...
    if (harvestTemplates.isEmpty) {
      return Text(
        'No harvest information available.',
        style: TextStyle(
          fontSize: 14,
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: harvestTemplates.map((template) {
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // Steps with leaf icons
              if (template['steps'] != null)
                Column(
                  children: (template['steps'] as List).map<Widget>((step) {
                    return Padding(
